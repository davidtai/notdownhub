...
export const INNER_INDEX_JS = [
  '// notdownhub inner localcheckout (#107): warning-free port of the engine\'s inner action.',
  ...
  'function setFailed(msg) { process.exitCode = 1; process.stdout.write("::error::" + esc(msg) + os.EOL); }',
  ...
  '        if (streamDone && openWriters === 0) handlers.done(sawTerminator);',
  '    }',
  ...
  '        // so both forms occur in one stream. filename*= is RFC 5987 and deliberately not matched.',
  '        var nameMatch = disp && /(?:^|;)\\s*name\\s*=\\s*(?:"([^"]*)"|([^;]*))/i.exec(disp[1]);',
  '        var fileMatch = disp && /(?:^|;)\\s*filename\\s*=\\s*(?:"([^"]*)"|([^;]*))/i.exec(disp[1]);',
  ...
  '        openWriters++;',
  '        part.ws.on("error", function (e) { warning("Failed to write `" + target + "`: " + e.message); });',
  '        part.ws.on("close", function () { openWriters--; maybeFinish(); });',
  '        debug(part.relpath + ", mode=" + part.mode + " => " + target);',
  '    }',
  '    function writeData(data) {',
  '        if (!part || data.length === 0) return true;',
  '        if (part.ws) {',
  '            var ok = part.ws.write(data);',
  '            if (!ok) {',
  '                stream.pause();',
  '                part.ws.once("drain", function () { stream.resume(); });',
  '            }',
  '            return ok;',
  '        }',
  '        if (!part.isFile) part.chunks.push(Buffer.from(data));',
  '        return true; // guarded-away file data is discarded',
  '    }',
  '    function endPart() {',
  '        if (!part) return;',
  '        if (part.ws) {',
  ...
  '                        if (!writeData(chunk)) return; // paused; drain resumes the stream',
  '                    }',
  '                    return;',
  '                }',
