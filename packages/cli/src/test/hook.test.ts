...
import { generateHook, shSingleQuote, installHook, HOOK_MARKER, __test } from "../hook.js";
...
test("generateHook: embeds server, loops refs/heads/*, dispatches with --ref, no -W by default", () => {
  const script = generateHook({ server: "http://hub.local:4949" });
  ...
  assert.ok(
    script.includes('ndh dispatch --server "$HUB" --event push --ref "$ref"'),
    "dispatches per pushed ref",
  );
...
test("generateHook: propagates the workflow flag when configured", () => {
  const script = generateHook({ server: "http://h:4949", workflow: ".github/workflows/ci.yml" });
  assert.ok(script.includes("WORKFLOW='.github/workflows/ci.yml'"), "single-quoted workflow var");
  assert.ok(
    script.includes('ndh dispatch --server "$HUB" --event push --ref "$ref" -W "$WORKFLOW"'),
    "adds -W to the dispatch",
  );
});

test("generateHook: a server url with a single quote stays single-quote-safe", () => {
  const script = generateHook({ server: "http://ho'st:4949" });
