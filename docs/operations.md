...
`ndh hub down` verifies process identity before it signals a pid. `ndh hub up`
records the start time and command line of each pid. `hub down` signals a pid
only when its live identity still matches. A reused pid never matches after a
crash, so `hub down` leaves that unrelated process alone and reports it.
